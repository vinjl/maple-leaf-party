# The Maple Leaf — First Principles & Constitution of Purpose

**Version:** 0.4  
**Status:** Framework draft  
**Party:** The Maple Leaf (Maple Leaf Party of Canada)

---

## 1. Purpose

The Maple Leaf exists to govern Canada by **maximum logic in the national interest**: the durable welfare, security, prosperity, and self-determination of **Canadian citizens**, under evidence, incentives, and the constraints of reality.

We are not a culture-war cosplay party, a corporate lobby vehicle, or a global charity with a flag. We are a **fiduciary of the citizen body**.

---

## 2. North-star identity

**Logic-based civic nationalism** + **Switzerland of North America**.

- **Civic nationalism:** the nation is citizens, law, territory, and mutual obligation — not ethnic purity and not borderless sentimentalism.
- **Swiss-NA economy:** hard fiscal credibility, capital magnet, world-class banking under Canadian law, low extraction, strong currency, open books, conditional citizen safety net.

**Brand promise:**  
*Earn in North America. Keep more of it. Bank it under Canadian law. Deploy it into Canadian productivity. Raise kids here. Sleep at night.*

---

## 3. Non-negotiables

1. **Citizens first.** State power is exercised for Canadians who fund and are bound by Canadian law. Foreign interests, temporary entrants, and abstract “global community” claims rank below citizens.
2. **Evidence over fashion.** Policy follows data, history, and incentive design — not donor memes, academic fads, or media pile-ons.
3. **Government as open source — 100% non-exempt.** Treat the state like an open source project: **transparent, open source, secure, lean, integrity-max.** Every non-exempt **outflow dollar** — including **all government benefits** — is ledger-traced and publicly auditable (**anonymized** for individual recipients; never dark). Every non-exempt process is versioned and inspectable; citizens file **bugs/fraud** on a public tracker (MCIP). **Only carve-outs:** military operations and secret police/intelligence operations.
4. **AI-maximized state — every non-exempt function.** World-class national and provincial extreme compute; AI watches the state for waste and abuse; **Human Council** over the machines; no social-credit panopticon.  
4d. **Universal Decision Logic.** Every material decision survives extreme, ideology-blind logic: citizen outcome, steelman for/against, replicable data, foreign-interest tests (`20`).
4b. **Free speech.** Criminal speech law covers **explicit violence calls/threats only** — not subjective hate or hurt feelings.
4c. **Nation of owners.** Maple Seed→Tree (RESP overhaul): deductible in, tax-free growth and out; Canada-first corporate tax when profits stay and pay Canadians.
5. **Hard Canadian dollar.** Minimal inflation; fiscal steel; major resources settled in **CAD** so the world must hold our currency; resource wealth saved, not binged.
6. **Spend before tax theatre.** Cut unnecessary spending and redesign expensive systems (especially healthcare basket) **before** permanent tax collapse — so markets trust the Swiss brand.
7. **Markets where they excel; state where they don’t.** Private free markets are default. The state handles public goods, catastrophe insurance, borders, justice, strategic capacity, and clear externalities.
8. **Capital is welcome on Canadian terms.** Clean global wealth may bank and build here. It does not buy citizenship toys, empty urban housing stock, or political capture.
9. **Capacity-limited immigration.** Seats are scarce. Prefer fully highly skilled, security-cleared people. Volume tracks housing, hospitals, wages, and integration bandwidth.
10. **Equality under law.** Colour-blind merit. No race spoils system in federal hiring or grants.
11. **Verifiable democracy.** Elections must be publicly auditable without destroying ballot secrecy.
12. **Constitutional seriousness.** Prefer policies designed to survive Charter and federalism scrutiny over podcast maximalism that dies in court.

---

## 4. Loyalty hierarchy (state action)

When interests conflict, rank:

1. Canadian **citizens** (including future citizens — children and fiscal sustainability)
2. Permanent residents (limited, contractual)
3. Temporary foreign nationals (narrow, reversible status)
4. Allied states (reciprocal interest)
5. Abstract foreign or transnational claims

---

## 5. Decision algorithm

Every bill, program, regulation, and party policy must pass the short tests below — and, for **material** decisions, the full **Universal Decision Logic** battery in [`20-universal-decision-logic.md`](20-universal-decision-logic.md).

| # | Test | Fail means |
|---|------|------------|
| 1 | **Citizen interest** — safer, smarter, stronger, richer for citizens **and their children**? | Reject or redesign |
| 2 | **Extreme logic** — steelman for *and* against; chains valid without fashion labels? | Reject or rewrite argument |
| 3 | **Evidence grade** — replicable, not foreign-captured, not single irreplicable study? | Reject or pilot with kill switch |
| 4 | **Foreign interest** — hidden foreign gain without citizen gain? | Reject or redesign |
| 5 | **History of money/risk** — prior spend failed? prior intake imposed harm? | Reallocate, redesign, or refuse |
| 6 | **Subsidiarity** — can markets/families/provinces do it better? | Devolve or don’t create |
| 7 | **Skin in the game** — payers and beneficiaries aligned? | Add conditions/clawbacks |
| 8 | **Auditability** — can a competent citizen verify cost & outcome? | Open-State redesign |
| 9 | **Sunset / kill criteria** — how does it die if it fails? | Mandatory sunset |
| 10 | **Constitutional realism** — winable design? | Least-impairing rewrite |
| 11 | **Capital & talent retention** — makes Canada beat the US on keep-rate? | Fix tax/regulatory stack |

AI State on the Maple Extreme Compute Grid runs these tests at scale; the **Human Council of Logic & Integrity** and named officials remain accountable (`08`).

---

## 6. What the party will never do

- Run deficits as a personality brand while claiming “strong dollar”
- Confiscatory surprise wealth raids (destroys capital-magnet credibility)
- Market Canada as a money-laundering or court-evasion haven
- Flood immigration while housing and hospitals are in shortage
- Promise free unlimited healthcare for preventable self-harm with no conditions **and** Swiss-level taxes — that is arithmetic fraud
- Ethnic scapegoating or racial spoils as substitutes for competence
- Close the books on government spending “for your safety” without narrow, logged exemptions
- Subordinate Canadian resource development to foreign activist capture that exports emissions and jobs to dirtier jurisdictions

---

## 7. Definition of the nation

**Canada** is a sovereign people on a specific territory, under a specific constitution, with official languages as practical infrastructure, and with a duty to remain solvent, defensible, and free for its citizens a century from now.

The Maple Leaf’s patriotism is **operational**: balance sheets, borders, birth rates of families who can afford kids, factories, pipelines, courts, and trust in elections — not empty flag cosplay.

---

## 8. Relationship to existing Maple pillar work

Legal/Charter pathways already in motion (e.g. evidence-based youth protection / developing-brain issues) are **proof-of-method**: follow evidence even when industry dislikes it. They are signature issues, not the whole party.

---

## 9. Amendment rule

Changes to these First Principles require:

1. Written argument memo (problem, evidence, alternatives, risks)
2. Version bump and public changelog
3. Founding Council / member ratification under party constitution (once formed)

No silent vibe-shifts.

---

## 10. One-sentence constitution of purpose

> **The Maple Leaf shall make Canada the most logical place on Earth for citizens to live free, keep what they earn, raise children, bank capital under hard law, and run government as an open source project — transparent, open source, secure, lean, integrity-max — verifying every non-exempt dollar and process on a public ledger (only military ops and secret police ops carved out), with world-leading AI under human accountability, and a hard dollar the world needs to buy Canadian resources — loyal to Canadians first, last, and always.**
